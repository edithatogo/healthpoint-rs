...
| Glama | Open-source server submission is from a GitHub repository with repository URL, display name, and short description; Glama performs automated quality checks including license detection, security scan, and health test; optional `glama.json` can control metadata/build/env. Connectors require HTTPS Streamable HTTP and optional test credentials unless OAuth dynamic client registration is implemented. Source: Glama FAQ and the public score page. | Posted with Glama release `v0.1.0` created from successful test `019f181f-0b4a-7200-b5a0-f52c6516b629` on 2026-06-30. Try in Browser launched Glama instance `o7bzzvjw41` and successfully executed `healthpoint_access_policy`. The public score improved from `25%` to `58%`; remaining visible gaps are pending tool scores, delayed usage recognition, and `No glama.json` on Glama's indexed copy. Hosted connector path is not ready because no public HTTPS Streamable HTTP endpoint exists. | Push the repo-local `glama.json` changes to `main`, sync the Glama repository index, and recheck after Glama finishes usage/index/tool-score processing. |
...
- [x] Glama repo listing posted, Glama release `v0.1.0` created from successful test `019f181f-0b4a-7200-b5a0-f52c6516b629`, and Try in Browser executed `healthpoint_access_policy`; the public score currently shows a 58% quality state with delayed usage/`glama.json` recognition and tool-score completion still incomplete.
