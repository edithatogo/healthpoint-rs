...
- Cargo package to register: `healthpoint-mcp`.
- Transport: `stdio`.
- Distribution path: Cargo/crates.io first, with GitHub Release binaries as an additional convenience distribution.
...
| --- | --- | --- | --- |
| Official MCP Registry | Uses `server.json`; registry only stores metadata; package must be published first; publisher CLI is `mcp-publisher`; GitHub auth requires an `io.github.<user>/...` server name; Cargo packages require `registryType: cargo`, crates.io identifier/version, stdio transport, and a visible `mcp-name: <server-name>` token in the README rendered by crates.io. Sources: official quickstart and package-types docs. | Manifest and README now satisfy the documented Cargo metadata shape locally. | Blocked until `healthpoint-mcp` exists on crates.io and `mcp-publisher login github && mcp-publisher publish` can be run. |
...
- [x] Cargo package entry for `healthpoint-mcp`.
...
- [ ] `healthpoint-mcp` published to crates.io.
- [ ] Official `mcp-publisher` installed and authenticated.
...
7. Verify official registry search for `io.github.edithatogo/healthpoint-rs`.
8. Submit account-gated directories and record accepted/rejected/pending status in issue #21.
