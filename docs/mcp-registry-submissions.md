...
| Glama | Open-source server submission is from a GitHub repository with repository URL, display name, short description, release metadata, README, license, maintenance/security signals, optional `glama.json`, related servers, and a generated tool-quality score. The visible score page says the quality score is `70% Tool Definition Quality + 30% Server Coherence`; Tool Definition Quality is `60% mean TDQS + 40% minimum TDQS` across Purpose Clarity, Usage Guidelines, Behavioral Transparency, Parameter Semantics, Conciseness/Structure, and Contextual Completeness. Server Coherence scores Disambiguation, Naming Consistency, Tool Count Appropriateness, and Completeness equally. Source: live Glama score page checked 2026-06-30. | Submitted and released. Live observed profile score is `83%`, latest Glama release is `v0.1.0`, Server Coherence is `A` with `5/5` on all four dimensions, Tool Definition Quality is `A` with average `3.7/5` across 10 tools and lowest `3.1/5`. Visible profile checks pass for release, maintenance, license, README, active usage, and author verification. Visible remaining gaps are `No glama.json` and `No related servers`; `glama.json` exists in this PR branch and should clear after merge/sync, while related servers are external directory metadata. `scripts/check-mcp-registry-submission.py` now emits a `glama_score_contract` block with this rubric and observed state. | Closed for initial submission/release. Repo-controlled next step is to merge `glama.json` to `main`, trigger Glama sync, and then improve tool descriptions/annotations if targeting a higher TDQS. Related servers remains manual/external directory curation. |
...
- [x] Local `glama.json` added and validated by `scripts/check-mcp-registry-submission.py`.
